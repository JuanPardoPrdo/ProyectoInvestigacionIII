...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const netResponse = await fetch(`http://localhost:5080/api/residentes/unidades/${id}`, {
            method: 'PUT',
            headers: {
        ...
        return NextResponse.json({ message: 'Unidad residencial actualizada con éxito' }, { status: 200 });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return NextResponse.json({ message: 'Unidad residencial eliminada' }, { status: 200 });
