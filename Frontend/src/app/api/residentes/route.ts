...
export async function GET(req: NextRequest) {
    try {
        ...
        const netResponse = await fetch('http://localhost:5080/api/residentes', {
            method: 'GET',
            headers: {
        ...
        return NextResponse.json({ residentes: data.residentes }, { status: 200 });

    } catch (err: any) {
        console.error('Proxy GET residentes error:', err);
...
export async function POST(req: NextRequest) {
    try {
        ...
        console.error('Proxy POST residente exception:', err);
        if (err.code === 'ECONNREFUSED') {
            return NextResponse.json(
